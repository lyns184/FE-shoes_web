              ...
              <div className="flex gap-2 flex-wrap">
                {[0, 1, 2, 3, 4].map((index) => (
                  <button
                    ...
                    className={`w-16 h-16 border rounded-lg overflow-hidden cursor-pointer transition-colors ${
                      selectedType === index
                        ? 'border-[#396254] border-2'
                        : 'border-gray-300 hover:border-gray-400'
                    }`}
                  >
                    <img src={product.imageUrl} alt="Type" className="w-full h-full object-contain p-1" />
                  </button>
                ))}
              </div>
            </div>
